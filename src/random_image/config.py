...
class Settings:
    ...
    # 性能配置
    MAX_IMAGE_SIZE: int = int(os.getenv("MAX_IMAGE_SIZE", "2000"))  # 最大图片尺寸（像素）
    ...
    def validate_config(self) -> None:
        ...
        if not (self.MIN_COMPRESSION_QUALITY <= self.DEFAULT_COMPRESSION_QUALITY <= self.MAX_COMPRESSION_QUALITY):
            raise ValueError(f"压缩质量必须在 {self.MIN_COMPRESSION_QUALITY}-{self.MAX_COMPRESSION_QUALITY} 之间")

...
class ImageProcessor:
    ...
    def calculate_optimal_quality(
        self, image_size: int, target_size: Optional[int] = None
    ) -> int:
        """根据图片大小计算最优压缩质量"""
    ...
    def resize_image(self, img: Image.Image, params: ImageRequestParams) -> Image.Image:
        ...
        # 限制最大尺寸
        if (
            new_size[0] > settings.MAX_IMAGE_SIZE
            or new_size[1] > settings.MAX_IMAGE_SIZE
        ):
    ...
    def convert_format(
        ...
        """转换图片格式"""
        # 处理RGBA模式的图片
        if img.mode in ("RGBA", "LA") and target_format in [
            ImageFormat.JPEG,
            ImageFormat.WEBP,
        ]:
            # 创建白色背景
    ...
    def compress_image(
        self, img: Image.Image, params: ImageRequestParams, original_size: int
    ) -> ProcessedImage:
        """压缩图片"""
        try:
            # 调整尺寸
            ...
            quality = max(
                settings.MIN_COMPRESSION_QUALITY,

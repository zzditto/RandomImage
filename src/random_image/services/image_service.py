...
from ..models import ImageInfo, ProcessedImage, ImageRequestParams, HealthStatus
from ..exceptions import ImageNotFoundError, DirectoryAccessError, ImageProcessingError
...
class ImageService:
            ...
            try:
                for file_path in directory.rglob("*"):
                    if (
                        ...
                        and image_processor.validate_image_format(file_path)
                    ):
    ...
    def process_image(
        self, image_path: Path, params: ImageRequestParams
    ) -> ProcessedImage:
        """处理图片（带缓存）"""
        # 生成缓存键
